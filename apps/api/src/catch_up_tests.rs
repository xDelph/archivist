...
#[tokio::test]
async fn catch_up_route_only_returns_threads_for_the_session_team() {
    let tempdir = tempdir().expect("tempdir");
    let path = tempdir.path().join("events.jsonl");
    let store = JsonlEventStore::open(&path).await.expect("store");
    let now = current_unix_timestamp();
    ...
    for (event_id, team_id, text) in [
        ("evt_t123", "T123", "visible root"),
        ("evt_t999", "T999", "hidden root"),
    ] {
        store
            .record_process_event(&ProcessEventJob {
    ...
    assert_eq!(
        payload["channels"][0]["threads"][0]["title"],
        "visible root"
    );
}
